...
def resolve_languages(df: pd.DataFrame, ui_lang: str = "en") -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        if code:
            langs.append(code)
            sources.append("campo Zotero")
        else:
            text = build_detection_text(row)
            code = detect_lang(text)
            langs.append(code)
            sources.append("detection" if code != "und" else "non determinata")
    out = df.copy()
    out["lang"] = langs
...
def main() -> None:
    ...
    type_table.to_csv(args.outdir / "1_tipologie.csv", index=False)
    ...
    donut(
        ...
        args.outdir / "1_donut_tipologie.html", ui,
    )
    print("1. TIPOLOGIE")
    ...
    lang_table.to_csv(args.outdir / "2_lingue.csv", index=False)

    donut(
        lang_counts, ui["t_langs"], ui["s_langs"].format(n=len(df)),
        args.outdir / "2_donut_lingue.html", ui,
    )
    print("2. LINGUE")
    ...
    freq.to_csv(args.outdir / "3_tag_vocabolario.csv", index=False)
    long.to_csv(args.outdir / "3_item_tag_long.csv", index=False)
    ...
    # Dataset arricchito, riutilizzabile in fase 2.
    df.to_csv(args.outdir / "0_dataset_arricchito.csv", index=False)
