...
rule(["università"], FACET, "University",
     nota="Assunto come target per coerenza con gli altri tre - VERIFICARE")

# --- L1: stoplist ----------------------------------------------------------
rule("no tag", DROP, nota="Placeholder, non descrittore (85 occorrenze)")
rule(["English", "French"], DROP, nota="Tag di lingua: ridondante rispetto al campo Language")
rule(["FAQ", "Manuale", "scientific report"], DROP,
     nota="Tipo documentale, ridondante rispetto a Item Type - VERIFICARE")
...
rule(_meta, DROP, nota="Metadato editoriale/arXiv importato, non indicizzazione umana")
...
rule(["NLP", "Natural Language Processing", "Natural laguage processing",
     ...
     nota="Include correzione del refuso 'laguage' (5 occorrenze)")
...
rule(["Sistemi di intelligenza artificiale"], MERGE, "AI systems",
     nota="Termine giuridico (AI Act): tenuto distinto da 'Artificial intelligence'")
...
rule(["Promp engineering"], MERGE, "Prompt engineering", nota="Correzione refuso")
rule(["Promp injection"], MERGE, "Prompt injection", nota="Correzione refuso")
rule(["pappagalli stocastici"], MERGE, "Stochastic parrots")
rule(["intelligenza condivisa"], MERGE, "Shared intelligence")
rule(["Technologica humanism"], MERGE, "Technological humanism", nota="Correzione refuso")
...
rule(["HTR", "Handwritten text recognition", "handwriting recognition",
     ...
     nota="Le tre varianti 'HTR for...' sono specificazioni di dominio del medesimo concetto - VERIFICARE")
...
rule(["OCR", "Optical character recognition OCR",
     ...
     "Optical character recognition", "OCR", nota="Include de-escaping LaTeX")
rule(["automatic transcription", "Transcription"], MERGE, "Automatic transcription",
     nota="Tenuto distinto da HTR: la trascrizione è il compito, HTR la tecnica - VERIFICARE")
...
rule(["Software libraries"], KEEP, "Software libraries",
     nota="Omonimia con 'Libraries': accezione informatica, NON accorpare")
...
rule(["MAB"], MERGE, "GLAM institutions", "MAB",
     nota="Trattato come oggetto (istituzioni MAB come tema), non come destinatario")
...
rule(["uso responsabile", "sviluppo responsabile", "Responsible AI"], MERGE, "Responsible AI",
     nota="Accorpa 'uso responsabile' e 'sviluppo responsabile' - VERIFICARE")
...
rule(["Distribute authorship"], MERGE, "Distributed authorship", nota="Correzione refuso")
rule(["Suspence"], MERGE, "Suspense", nota="Correzione refuso")
rule(["Multilingal information retrieval", "Recherche d'information multilingue"], MERGE,
     "Multilingual information retrieval", nota="Include correzione refuso 'Multilingal'")
...
rule(["Old prints", "Imprim{\\'e}s anciens"], MERGE, "Early printed books",
     nota="Include de-escaping LaTeX")
...
def build_mapping_table(long: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in vocab.iterrows():
        ...
        rows.append({
            "tag_originale": r.tag_originale,
            "occorrenze": r.occorrenze,
            "origine": r.origine,
            "azione": azione,
            "concetto_canonico": canonico if azione != DROP else "",
            "etichetta_grafo": etichetta,
            "nota": nota,
        })
    return (pd.DataFrame(rows)
            .sort_values(["occorrenze", "concetto_canonico"], ascending=[False, True])
...
def apply_mapping(long: pd.DataFrame, mapping: pd.DataFrame,
    ...
    """
    idx = {r.tag_originale.lower(): r for _, r in mapping.iterrows()}
    rows, facets = [], []
    for _, r in long.iterrows():
        m = idx.get(r.tag_key)
        if m is None or m.azione == DROP:
            continue
        if m.azione == FACET:
            facets.append({"Key": r.Key, "Title": r.Title,
                           "target": m.concetto_canonico, "tag_originale": r.tag})
            continue
        rows.append({"Key": r.Key, "Title": r.Title,
                     "concetto": m.concetto_canonico,
                     "etichetta": m.etichetta_grafo or m.concetto_canonico,
                     "tag_originale": r.tag})

    out = pd.DataFrame(rows).drop_duplicates(subset=["Key", "concetto"])  # L2: dedup post-merge
    target = (pd.DataFrame(facets, columns=["Key", "Title", "target", "tag_originale"])
              .drop_duplicates(subset=["Key", "target"]))

    if min_freq > 1:  # L3
        keep = out.groupby("concetto")["Key"].nunique()
        out = out[out.concetto.isin(keep[keep >= min_freq].index)]
    return out.reset_index(drop=True), target.reset_index(drop=True)


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--long", required=True, type=Path, help="3_item_tag_long.csv")
    ap.add_argument("--outdir", default=Path("output"), type=Path)
    ap.add_argument("--mapping", type=Path, help="mapping.csv revisionata (opzionale)")
    ...
    if args.mapping:
        ...
        mapping.to_csv(args.outdir / "4_mapping.csv", index=False)

    norm, target = apply_mapping(long, mapping, args.min_freq)
    norm.to_csv(args.outdir / "5_item_concetto_long.csv", index=False)
    target.to_csv(args.outdir / "6_item_target.csv", index=False)

    # --- Report -------------------------------------------------------------
    n_facet = (mapping.azione == FACET).sum()
    n_drop = (mapping.azione == DROP).sum()
    n_merge = (mapping.azione == MERGE).sum()
    n_keep = (mapping.azione == KEEP).sum()
    flagged = mapping[mapping.nota.str.contains("VERIFICARE", na=False)]
    ...
    print(f"Concetti dopo L0-L2: {post_all.concetto.nunique()} (da {pre_concepts} tag sostanziali)")
    print(f"Concetti dopo L3 (freq >= {args.min_freq}): {norm.concetto.nunique()}")
    ...
    hapax_post = (post_all.groupby("concetto").Key.nunique() == 1).sum()
    print(f"Hapax: {hapax_pre} -> {hapax_post} "
          f"({hapax_post / post_all.concetto.nunique() * 100:.0f}% del vocabolario)")
    ...
    for _, r in flagged.iterrows():
        print(f"  [{r.occorrenze:>2}] {r.tag_originale} -> {r.concetto_canonico}\n"
              f"       {r.nota}")

    print("\nPRIMI 20 CONCETTI:")
    top = norm.groupby("concetto").Key.nunique().sort_values(ascending=False).head(20)
    print(top.to_string())
